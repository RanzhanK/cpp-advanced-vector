...
namespace {
    ...
    struct TestObj {
        ...
        [[nodiscard]] bool IsAlive() const noexcept { return cookie == DEFAULT_COOKIE; }
    ...
    struct Obj {
        ...
        explicit Obj(int id) : id(id) { ++num_constructed_with_id; }

        Obj(int id, std::string name) : id(id), name(std::move(name)) { ++num_constructed_with_id_and_name; }
        ...
        Obj(Obj &&other) noexcept: id(other.id) { ++num_moved; }
...
void Test6() {
    ...
    }
    {
        ...
        assert(std::all_of(v.begin(), v.end(), [](const TestObj &obj) { return obj.IsAlive(); }));
    }
...
struct C {
    ...
    C(const C & /*other*/) noexcept { ++copy_ctor; }

    C(C && /*other*/) noexcept { ++move_ctor; }
